<div class="p-6 space-y-6">
    <h1 class="text-xl font-bold">Create Purchase Return</h1>
    <flux:separator class="mb-4"></flux:separator>
    {{-- Search Input --}}
    @if (!$selectedPurchase)
        <div class="flex justify-between items-center mb-4">
            <input type="text" wire:model.live.debounce.300ms="search"
                placeholder="Search by purchase code or supplier..." class="border rounded p-2 w-1/3">
        </div>
        ...
        <table class="w-full border text-sm">
            <thead class="bg-gray-100">
                <tr>
                    <th class="p-2 text-left">Kode Pembelian</th>
            ...
            <tbody>
                @forelse ($purchases as $p)
                    <tr class="border-t hover:bg-gray-50">
                        <td class="p-2">{{ $p->purchase_code }}</td>
                        <td class="p-2">{{ $p->supplier->supplier_name ?? '-' }}</td>
                        <td class="p-2">{{ \Carbon\Carbon::parse($p->purchase_date)->format('d M Y') }}</td>
                        <td class="p-2">{{ number_format($p->total_amount, 2) }}</td>
                        <td class="p-2 text-center">
                            <button wire:click="selectPurchase({{ $p->id }})"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center p-4 text-gray-500">Tidak ada data pembelian.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($selectedPurchase)
        ...
        <div>
            <h2 class="font-semibold mb-2">
                Detail Pembelian ({{ $selectedPurchase->purchase_code }})
            </h2>
            ...
            <table class="w-full border text-sm">
                ...
                <tbody>
                    @foreach ($selectedPurchase->details as $detail)
                        <tr class="border-t">
                ...
                <table class="w-full border text-sm">
                    ...
                    <tbody>
                        @foreach ($selectedItems as $detailId => $item)
                            @php $detail = $selectedPurchase->details->firstWhere('id', $detailId); @endphp
                            <tr class="border-t">
                                <td class="p-2 text-center">{{ $detail->item->item_name }}</td>
                                <td class="p-2 text-center">
