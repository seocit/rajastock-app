<flux:modal name="return-details" wire:model="isOpen" dismissible
    ...
    @if ($returnData)
        <div class="space-y-3">
            <flux:heading size="xl" class="mb-6 text-center md:text-left">Return Details</flux:heading>
            ...
            <div class="flex flex-wrap text-sm gap-x-8 gap-y-3 sm:gap-x-6 md:gap-x-10">
                ...
                <div class="flex">
                    <div><strong>Purchase Code:</strong></div>
                    <div class="pl-2">{{ $returnData->purchase->purchase_code ?? '-' }}</div>
                </div>
                    ...
                    <tbody>
                        @foreach ($details as $d)
                            <tr class="border-t hover:bg-gray-50">
                                <td class="p-2">{{ $d->purchaseDetail->item->item_name ?? '-' }}</td>
